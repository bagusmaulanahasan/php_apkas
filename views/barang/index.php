<?php
	...
	include base_project()."controllers/barangController.php";

	$barangController = new BarangController();
	$dataBarang = $barangController->getAllBarang();


	$mode = isset($_GET['edit']) ? 'edit' : 'add';
	$data = $mode === 'edit' ? $barangController->getBarangById($_GET['edit']) : null;
...
<body class="px-8 ">
	...
	<button id="tambahBarang" class="px-4 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-700 cursor-pointer mt-6 ms-2">Tambah Barang</button>
    <table border="1" cellspacing="0" class="border w-2/3 mx-auto">
        <caption class="text-lg font-semibold mb-2">Daftar Barang</caption>
        <thead>
			<tr class="bg-blue-200 text-slate-800">
                <th class="py-3 ps-2 text-start">Nama Barang</th>
                <th class="py-3 ps-2 text-start">Harga</th>
                <th class="py-3 ps-2 text-center w-64">Aksi</th>
             </tr>
		</thead>
		<tbody>
            <?php 
        ...
        <form method="post" action="<?= $action ?>" class="bg-white p-4 rounded-lg w-1/3">
            <fieldset class="border border-solid flex flex-col p-4 justify-center items-start gap-6 rounded-xl <?= $mode === 'edit' ? 'border-green-600' : 'border-blue-600' ?>">
                ...
                <div class="flex w-full">
                    ...
                    <input required type="text" name="nama_barang" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['nama_barang'] ?? '' ?>">
                </div>
                <div class="flex w-full">
                    <label class="w-48">Harga</label>
                    <input required type="text" name="harga" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['harga'] ?? '' ?>">
                </div>
    ...
    <script>
        ...
        btnTambahBarang.addEventListener('click', (e) => {
            e.preventDefault();
            form.classList.remove('hidden');
            form.classList.add('flex');
        })
