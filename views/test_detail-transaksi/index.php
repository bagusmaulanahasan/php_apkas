...
$dataDetailTransaksi = $DetailTransaksiController->getAllDetailTransaksi();

$mode = isset($_GET['edit']) ? 'edit' : 'add';
$data = $mode === 'edit' ? $DetailTransaksiController->getDetailTransaksiById($_GET['edit']) : null;
...
<head>
    ...
    <title>DetailTransaksi</title>
    <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
</head>

<body class="px-8">
    <?php include "../layouts/navbar.php" ?>
    <button id="btnTambahDetailTransaksi" class="px-4 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-700 cursor-pointer mt-6 ms-2">Tambah Detail Transaksi</button>
    <table border="1" cellspacing="0" class="border w-2/3 w-full mx-auto">
        <caption class="text-lg font-semibold mb-2">Daftar Detail Transaksi</caption>
        <thead>
            <tr class="bg-blue-200 text-slate-800">
                <th class="py-3 ps-2 text-start">Nama Barang</th>
                <th class="py-3 text-start">ID Transaksi</th>
                <th class="py-3 text-start">Harga Barang</th>
                <th class="py-3 text-start">Jumlah Barang</th>
            ...
            <?php 
                foreach ($data as $row) : ?>
                    <tr class="even:bg-gray-100 text-slate-800">
                        ...
                        <td class="py-3"><?=$row['harga_barang']?></td>
                        <td class="py-3"><?=$row['jumlah_barang']?></td>
        ...
        <form method="post" action="<?= $action ?>" class="bg-white p-4 rounded-lg w-1/3">
            <fieldset class="border border-solid flex flex-col p-4 justify-center items-start gap-6 rounded-xl <?= $mode === 'edit' ? 'border-green-600' : 'border-blue-600' ?>">
                ...
                <div class="flex w-full">
                    <label class="w-48">Nama Barang</label>
                    <input required type="text" name="nama" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['nama_barang'] ?? '' ?>">
                </div>
                <div class="flex w-full">
                    <label class="w-48">ID Transaksi</label>
                    <input required type="text" name="alamat" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['id_transaksi'] ?? '' ?>">
                </div>
                <div class="flex w-full">
                    <label class="w-48">Harga Barang</label>
                    <input required type="text" name="alamat" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['harga_barang'] ?? '' ?>">
                </div>
                <div class="flex w-full">
                    <label class="w-48">Jumlah Barang</label>
                    <input required type="text" name="alamat" class="border border-gray-300 text-slate-600 rounded-sm w-full py-1 px-2 focus:border-gray-600 focus:outline-0" value="<?= $data['jumlah_barang'] ?? '' ?>">
                </div>
    ...
    <script>
        ...
        const btTambahDetailTransaksi = document.querySelector("#btTambahDetailTransaksi");
        const btnCancel = document.querySelector("#batalTambah");

        btnTambahDetailTransaksi.addEventListener('click', (e) => {
            e.preventDefault();
            form.classList.remove('hidden');
            form.classList.add('flex');
        })
